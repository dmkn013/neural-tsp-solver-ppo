...
class SupervisedTrainer():
    ...
    def fit(self, n_epochs):
        ...
        if not self.resume:
            with open(self.log_csv, 'w') as f:
                f.write('epoch,cost_train,cost_val,time_train,time_val\n')
    ...
    def train_epoch(self, i_epoch):
        ...
        losses = []
        train_dataset = TSPDataset(n_instances=self.epoch_size, prepare_target=True)
        ...
        for batch in tqdm(train_loader, postfix=f'{i_epoch}-th epoch processing'):
            points, target = batch
            loss, cost = self.train_batch(points, target)
            losses.append(loss)
            costs.append(cost)
    ...
    def train_batch(self, points, target):
        points = points.cuda()
        target = target.cuda()
        ...
        for i_step in range(n_nodes):
            log_p = self.model(points, target, i_step=i_step)
            loss = self.calc_loss(log_p, target, i_step) # (batch)
    ...
    def calc_loss(self, log_p, tour_target, i_step):
        '''
        log_p: (batch, n_nodes)
        target: (batch, n_nodes)
        '''
        batch_id = torch.arange(log_p.size(0))
        index_target = tour_target[:, i_step] # (batch)
        log_p_selected = log_p[batch_id, index_target] # (batch)
        return log_p_selected.mean()
